...
def compute_weighted_score(score: dict) -> dict:
    ...
    for metric_name, weight in label_weights.items():
        ...
        if metric_value is not None:
            ...
            logger.warning(f"⚠️ La métrica '{metric_name}' no está presente en el score para la etiqueta '{label}'.")
            # Si la métrica no está presente, se ignora
...
def main():
    try:
        ...
        for combination in combinations:
            print(f"Evaluando combinación {count+1}/{total_count}: {combination}")

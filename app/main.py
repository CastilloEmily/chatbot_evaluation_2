...
# Cargar variables de entorno desde configs/.env
...
from utils.change_settins import update_webapp_settings  # <- nombre tal cual lo tienes
...
def parse_args():
    ...
    parser.add_argument(
        '--mode',
        choices=["combinations", "current"],
        default="combinations",
        help="Modo de ejecución: 'combinations' itera y evalúa combinaciones; 'current' solo evalúa la configuración actual del Web App"
    )
...
def load_parameter_combinations(config_path):
    """
    1) Si existe 'combinations_pending.json', cargar de ahí.
    2) Si no, generar a partir de 'settings_ranges.yaml' y guardar en pending.
    """
    if os.path.exists(pending_file):
        ...
        num_samples = int(os.environ.get("NUM_SAMPLES", 4))  # Default 4
...
def generate_parameter_combinations(config_path, num_samples=4):
    """
    Genera todas las combinaciones a partir del YAML de rangos.
    Añade un 'id' a cada combinación.
    """
...
def evaluate_label(label, questions, config):
    ...
    for question in questions:
        ...
        per_question_scores.append(label_score)

    combination_score = calculate_average(per_question_scores)
...
def evaluate_question(question, config, label):
    qid = question['id']
    # En tu versión más reciente el campo era 'query' (no 'question')
    ...
    logger.info(f"Pregunta {qid} evaluada: {result['weighted_score']} para la etiqueta '{label}'")
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                raise  # Propaga para que main lo capture
...
def compute_weighted_score(score: dict) -> dict:
    ...
    for metric_name, weight in label_weights.items():
        ...
        if metric_value is not None and not (isinstance(metric_value, float) and math.isnan(metric_value)):
            ...
            logger.warning(
                f"⚠️ La métrica '{metric_name}' es inválida (None o NaN) y será ignorada para la etiqueta '{label}'."
            )
...
def main():
    ...
    elapsed = None
    reporter = None
    count = 0  # para mensajes de error
    try:
        args = parse_args()
        ...
        if args.mode == "current":
            ...
            # Modo combinations: iterar y evaluar combinaciones, mantener pending/completed
            combinations = load_parameter_combinations(config_path)
            combinations_pending = combinations[:]  # copia superficial
            ...
            for combination in combinations:
                ...
                _ = evaluate_combination(combination)

                # Añadir al archivo de completados
                with open(completed_file, "a", encoding="utf-8") as f_completed:
                    f_completed.write(json.dumps(combination, ensure_ascii=False) + "\n")

                # Quitar por ID real de la combinación, no por el contador
                combinations_pending = [c for c in combinations_pending if c["id"] != combination["id"]]

                # Sobrescribir el archivo de pendientes actualizado
                with open(pending_file, "w", encoding="utf-8") as f_pending:
                    json.dump(combinations_pending, f_pending, ensure_ascii=False)

                logger.info(f"Evaluación completada para la configuración: {combination}")
                count += 1

            # Eliminar pending si quedó vacío
            if os.path.exists(pending_file):
                try:
                    with open(pending_file, "r", encoding="utf-8") as f:
                    ...
                    # si hay corrupción, no se elimina
                    pass

        # Reempaquetar el NDJSON de combinaciones a una lista JSON robustamente
        data = []
        if os.path.exists(output_path_by_combination):
                ...
                for line in f:
                    ...
                    if line:
                        try:
                            ...
                            logger.warning("Línea inválida en output_path_by_combination; se ignora.")
        ...
        # Respetar tu firma de reporte en error:
        if reporter:
